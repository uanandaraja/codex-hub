...
const config: GatewayConfig = {
  ...
  defaultCwd:
    process.env.DEFAULT_THREAD_CWD ??
    resolve(import.meta.dirname, "..", "..", ".."),
  projectsRoot: process.env.PROJECTS_ROOT ?? join(homedir(), "Dev"),
  defaultApprovalPolicy: parseApprovalPolicy(
    process.env.DEFAULT_APPROVAL_POLICY,
  ),
  defaultSandboxMode: parseSandboxMode(process.env.DEFAULT_SANDBOX_MODE),
  autoApproveServerRequests: parseBoolean(
    process.env.AUTO_APPROVE_SERVER_REQUESTS,
  ),
};

const bridge = new CodexAppServerBridge(config);
const threadUsageCache = new Map<
  string,
  { modifiedAtMs: number; usage: ThreadUsageResponse }
>();
const threadListCache = new Map<
  string,
  { expiresAt: number; threads: CodexThread[] }
>();
...
async function handleRequest(
  ...
  try {
    ...
    if (method === "GET" && pathname === "/v1/threads") {
      await bridge.ensureReady();
      const projectPath = readOptionalString(
      ...
      if (projectPath) {
        const allThreads = sortThreads(
          (
            ...
            (thread) =>
              projectPathForThread(thread) === bridge.resolvePath(projectPath),
          ),
        );
        const offset = readOptionalNumber(url.searchParams.get("cursor")) ?? 0;
        const limit =
          readOptionalNumber(url.searchParams.get("limit")) ??
          allThreads.length;
        const data = allThreads
          .slice(offset, offset + limit)
          .map(compactThreadSummary);
        const nextCursor =
          offset + limit < allThreads.length ? `${offset + limit}` : null;
      ...
      const params = {
        ...
        cwd: url.searchParams.get("cwd")
          ? bridge.resolvePath(url.searchParams.get("cwd"))
          : null,
      };
      const data = await bridge.request<CodexThreadListResponse>(
        "thread/list",
        params,
      );
      return sendJson(response, 200, {
        ...data,
        data: data.data.map(compactThreadSummary),
    ...
    if (method === "POST" && pathname === "/v1/threads") {
      ...
      const payload = {
        ...
        approvalPolicy:
          readOptionalString(body.approvalPolicy) ??
      ...
      const result = await bridge.request<Record<string, unknown>>(
        "thread/start",
        payload,
      );
    ...
    const threadEventsMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/events$/,
    );
    ...
    if (method === "POST" && threadTurnsMatch) {
      ...
      if (!message && attachments.length === 0 && mentions.length === 0) {
        return sendJson(response, 400, {
          error: {
            message: "message, file mention, or image attachment is required",
          },
        });
      }
      ...
      const result = await bridge.request<Record<string, unknown>>(
        "turn/start",
        {
          ...
          cwd: readOptionalString(body.cwd)
            ? bridge.resolvePath(readOptionalString(body.cwd))
            : undefined,
    ...
    if (method === "GET" && pathname === "/v1/fuzzy-file-search") {
      ...
      const roots = url.searchParams
        ...
        .filter(
          (root, index, list) =>
            root.length > 0 && list.indexOf(root) === index,
        );
      ...
      const result = await bridge.request<FuzzyFileSearchResponse>(
        "fuzzyFileSearch",
        {
          query,
          roots,
    ...
    const threadServerRequestsMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/server-requests$/,
    );
    ...
    const threadInterruptMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/interrupt$/,
    );
    ...
    const threadGenerateNameMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/generate-name$/,
    );
    if (method === "POST" && threadGenerateNameMatch) {
      await bridge.ensureReady();
      const threadId = decodeURIComponent(threadGenerateNameMatch[1]);
      const result = await generateThreadName(threadId);
      invalidateThreadListCache();
      return sendJson(response, 200, result);
    }

    const threadArchiveMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/archive$/,
    );
    ...
    const threadUnarchiveMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/unarchive$/,
    );
    ...
    const threadResumeMatch = pathname.match(
      /^\/v1\/threads\/([^/]+)\/resume$/,
    );
    ...
    if (method === "GET" && threadUsageMatch) {
      ...
      const thread = await bridge.request<Record<string, unknown>>(
        "thread/read",
        {
          threadId,
          includeTurns: false,
    ...
    if (method === "GET" && threadMatch) {
      ...
      const includeTurns =
        readOptionalBoolean(url.searchParams.get("includeTurns")) ?? true;
      const includeUsage =
        readOptionalBoolean(url.searchParams.get("includeUsage")) ?? true;
      const tailTurns = normalizeTailTurns(
        readOptionalNumber(url.searchParams.get("tailTurns")),
      );
      const result = await bridge.request<
        Record<string, unknown> & { thread: CodexThread }
      >("thread/read", {
        threadId,
        includeTurns,
      });
      const { thread, truncatedTurnCount } =
        includeTurns && result.thread
          ? truncateThreadTurns(result.thread, tailTurns)
          : { thread: result.thread, truncatedTurnCount: 0 };
      return sendJson(response, 200, {
        ...result,
        thread,
        usage: includeUsage
          ? await readThreadUsageResponse(result)
          : EMPTY_THREAD_USAGE,
        truncatedTurnCount,
    ...
    if (method === "GET" && pathname === "/v1/fs") {
      ...
      if (metadata.isDirectory) {
        const result = await bridge.request<{ entries: DirectoryEntry[] }>(
          "fs/readDirectory",
          { path },
        );
      ...
      if (metadata.isFile) {
        const result = await bridge.request<{ dataBase64: string }>(
          "fs/readFile",
          { path },
        );
...
async function listAllThreads(options: {
  ...
  do {
    const page: CodexThreadListResponse =
      await bridge.request<CodexThreadListResponse>("thread/list", {
...
async function listAllModels(): Promise<ModelListResponse> {
  ...
  do {
    const page: ModelListResponse = await bridge.request<ModelListResponse>(
      "model/list",
      {
        limit: THREAD_PAGE_SIZE,
        cursor,
...
function truncateThreadTurns(
  thread: CodexThread,
  tailTurns: number | null,
): { thread: CodexThread; truncatedTurnCount: number } {
  if (tailTurns === null || thread.turns.length <= tailTurns) {
    return { thread, truncatedTurnCount: 0 };
  }

  const truncatedTurnCount = Math.max(0, thread.turns.length - tailTurns);
  return {
    thread: {
      ...thread,
      turns: tailTurns === 0 ? [] : thread.turns.slice(-tailTurns),
    },
    truncatedTurnCount,
  };
}

function normalizeTailTurns(value: number | null): number | null {
  if (value === null) {
    return null;
  }
...
function invalidateThreadListCache(): void {
...
function readTurnImageUploads(value: unknown): TurnImageUpload[] {
  ...
  return value.map((entry, index) => {
    ...
    const name =
      readOptionalString((entry as Record<string, unknown>).name) ??
      `image-${index + 1}`;
    const type = readOptionalString((entry as Record<string, unknown>).type);
    const dataBase64 = readOptionalString(
...
function extensionForUploadedImage(
  fileName: string,
  mimeType: string,
): string | null {
...
function mimeTypeForImagePath(path: string): string {
  return (
    IMAGE_EXTENSION_TO_MIME[extname(path).toLowerCase()] ??
...
async function readThreadUsageResponse(
  threadReadResult: Record<string, unknown>,
): Promise<ThreadUsageResponse> {
...
async function generateThreadName(
  threadId: string,
): Promise<ThreadNameGenerateResponse> {
  const result = await bridge.request<
    Record<string, unknown> & { thread: CodexThread }
  >("thread/read", {
    threadId,
  ...
  const generated = sanitizeGeneratedThreadName(
    await requestGeneratedThreadName(
      thread,
      context.userText,
...
function fallbackThreadName(thread: CodexThread): string | null {
  ...
  const firstClause =
    normalized.split(/[.!?](?:\s|$)/, 1)[0]?.trim() ?? normalized;
...
async function requestGeneratedThreadName(
  ...
  const promptThread = await bridge.request<
  ...
  const promptThreadId =
    typeof promptThread.thread === "object" &&
    promptThread.thread &&
    "id" in promptThread.thread
  ...
  try {
    ...
    unsubscribe();
    await bridge
      .request("thread/unsubscribe", { threadId: promptThreadId })
      .catch(() => {});
  }
}
...
async function readCachedThreadUsage(
  threadPath: string,
): Promise<ThreadUsageResponse> {
  try {
...
async function parseThreadUsage(
  threadPath: string,
): Promise<ThreadUsageResponse> {
  ...
  try {
    for await (const line of lines) {
      ...
      if (eventType === "task_started") {
        ...
        const contextWindow = readOptionalFiniteNumber(
...
function formatGatewayProviderLabel(
...
async function readLatestGatewayRateLimits(): Promise<{
  ...
  const recentSessionFiles = await listRecentSessionFiles(
    CODEX_SESSIONS_ROOT,
    12,
  );
  for (const sessionFile of recentSessionFiles) {
    const snapshot = await parseLatestRateLimitsFromSessionFile(
      sessionFile.path,
    );
    if (snapshot) {
      return snapshot;
...
async function parseLatestRateLimitsFromSessionFile(
  sessionPath: string,
): Promise<{
  planType: string | null;
  fiveHourLimit: GatewayAccountRateLimitWindow | null;
  ...
  try {
    for await (const line of lines) {
      ...
      latestSnapshot = {
        ...
        fiveHourLimit: selectRateLimitWindow(
          primaryWindow,
          secondaryWindow,
          300,
          "primary",
        ),
        weeklyLimit: selectRateLimitWindow(
          primaryWindow,
          secondaryWindow,
          10_080,
          "secondary",
...
function selectRateLimitWindow(
  ...
  const match = [primary, secondary].find(
    (window) => window?.windowMinutes === targetWindowMinutes,
  );
  if (match) {
    return match;
  }
...
function readThreadSessionPath(
  threadReadResult: Record<string, unknown>,
): string | null {
  const thread = isRecord(threadReadResult.thread)
